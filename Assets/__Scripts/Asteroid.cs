    ...
    public void OnCollisionEnter(Collision coll)
    {
        ...
        if (otherGO.CompareTag("Bullet") || otherGO.transform.root.gameObject.CompareTag("Player"))
        {
            ...
            if (size > 1)
            {
                if (size == 3)
                {
                    AsteraX.Instance.AddScore(100);
                }
                else if (size == 2)
                {
                    AsteraX.Instance.AddScore(200);
                }
                ...
                {
                    child.immune = true;
                    if (child == this || child.transform.parent != transform)
                    {
                        continue;
                    }
                    child.transform.SetParent(null, true);
                    child.InitAsteroidParent();
                }
            }

            if (size == 1)
            {
                AsteraX.Instance.AddScore(300);
            }
